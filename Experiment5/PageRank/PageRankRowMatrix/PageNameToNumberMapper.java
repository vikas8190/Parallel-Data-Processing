...
public class PageNameToNumberMapper extends Mapper <IntWritable, MapWritable, WritableComparable,Writable>{
    ...
    public void createOffsetsSplit(FileStatus[] files, Configuration conf) throws IOException{
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (currFile.toString()
                    .contains(PageRankRowMatrix.PrePrcoessOutputDir + PageRankRowMatrix.OffsetSubDir)) {
    ...
    public void cleanup(Context ctx) throws IOException, InterruptedException{
        mouts.write("dummy", NullWritable.get(), this.pageNameToNumber, PageRankRowMatrix.StringToNumberFilePathPrefix);
        mouts.write(PageRankRowMatrix.NumberToStringID, NullWritable.get(), this.pageNumberToString,
                PageRankRowMatrix.NumberToStringPathPrefix);
        mouts.close();
    }
