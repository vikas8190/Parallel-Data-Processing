...
public class SparseMatrixAggrReducer extends Reducer<LongWritable, DoubleWritable, LongWritable, DoubleWritable> {
	...
	public void cleanup(Context ctx) throws IOException, InterruptedException{
		...
		if(reduceId == 0){
			ctx.getCounter(PageRankColMatrix.counter.delta).setValue(Double.doubleToLongBits(this.danglingMass));
		}
	}
	...
	private void CalculateDanglingMass(FileStatus[] files, Configuration conf, Context context) throws IOException {
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (currFile.toString().contains(PageRankColMatrix.danglingContribSubDir)) {
