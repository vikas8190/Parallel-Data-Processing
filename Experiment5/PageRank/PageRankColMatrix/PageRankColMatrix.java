...
//Driver Class
public class PageRankColMatrix {
    ...
    static String columnContribFileID = "Column";
    static String colContributionSubDir = "/column";
    static String colContributionPathPrefic = "column/column";
    static String AggregatorOutputPath = "outputAggregator";
    static String danglingContribPathPrefix = "danglingMass/mass";
    static String danglingContribSubDir = "/danglingMass";
    static String danglingMassFileID = "Dangling";
    ...
    public static void main(String args[]) throws IOException{
        ...
        try {
            ...
            for(int i=0;i<10;i++){
                ...
                Job matrixAggregatorJob =  createMatrixAggregatorJob(conf, i);
                matrixAggregatorJob.waitForCompletion(true);
                delta=Double.longBitsToDouble(matrixAggregatorJob.getCounters().findCounter(counter.delta).getValue());
    ...
    public static Job createTopKJob(String[] remainingArgs,Configuration conf) throws IOException{
        ...
        job.setJarByClass(PageRankColMatrix.class);
        ...
        FileInputFormat.addInputPath(job, new Path(AggregatorOutputPath + "9"));
    ...
    public static Job createpreprocessJob(String[] remainingArgs,Configuration conf) throws IOException{
        ...
        MultipleOutputs.addNamedOutput(job,AdjListFileID,SequenceFileOutputFormat.class,Text.class,PageNode.class);
    ...
    public static Job createSparseMatrix(Configuration conf) throws IOException, URISyntaxException{
        ...
        System.out.println("Sparse matrix create");
        job.addCacheFile(new Path(PageNameToNumberOutputDir  +  stringToMapInput).toUri());

        job.setJarByClass(PageRankColMatrix.class);
        job.setReducerClass(SparseColMatrixReducer.class);
        job.setMapperClass(SparseColMatrixMapper.class);
        job.setMapOutputKeyClass(LongWritable.class);
    ...
    public static Job createMatrixMultiplyJob(Configuration conf, int iteration) throws IOException, URISyntaxException{
        ...
        System.out.println("Matrix multiply job:");
        MultipleInputs.addInputPath(job, new Path(SpareMatrixOutputDir + MatrixSubDir),
                SequenceFileInputFormat.class, SparseMatrixMulMapper.class);
        MultipleInputs.addInputPath(job, new Path(SpareMatrixOutputDir + DanglingNodeSubDir),
                SequenceFileInputFormat.class, DanglingNodeMapper.class);
        job.addCacheFile(new Path(SpareMatrixOutputDir +  "/dangling").toUri());
        if(iteration > 0){
            MultipleInputs.addInputPath(job, new Path(AggregatorOutputPath + (iteration - 1)),
                    SequenceFileInputFormat.class, PageRankMapper.class);
        }
        job.setMapOutputKeyClass(LongWritable.class);
        job.setMapOutputValueClass(ColumnContrib.class);
        job.setJarByClass(PageRankColMatrix.class);
        ...
        job.setReducerClass(SparseMatrixMulReducer.class);
        job.setOutputFormatClass(SequenceFileOutputFormat.class);
        MultipleOutputs.addNamedOutput(job, columnContribFileID, SequenceFileOutputFormat.class,
                LongWritable.class, DoubleWritable.class);
        MultipleOutputs.addNamedOutput(job, danglingMassFileID,
    ...
    public static Job createMatrixAggregatorJob(Configuration conf, int iteration) throws IOException, URISyntaxException{
        Job job = Job.getInstance(conf, "Matrix Aggregator Job");
        System.out.println("Aggregator job:");
        job.addCacheFile(new Path(PrePrcoessOutputDir + iteration + danglingContribSubDir).toUri());
        job.setJarByClass(PageRankColMatrix.class);
        job.setMapperClass(SparseMatrixAggrMapper.class);
        job.setReducerClass(SparseMatrixAggrReducer.class);
        ...
        job.setInputFormatClass(SequenceFileInputFormat.class);
        job.setOutputFormatClass(SequenceFileOutputFormat.class);
        FileInputFormat.addInputPath(job, new Path(PrePrcoessOutputDir + iteration + colContributionSubDir));
        FileOutputFormat.setOutputPath(job, new Path(AggregatorOutputPath + iteration));
        return job;
    }
